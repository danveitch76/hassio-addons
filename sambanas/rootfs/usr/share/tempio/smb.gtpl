...
# recycle - Moves files to a temporary directory rather than deleting them immediately.
   vfs objects = catia fruit streams_xattr{{ if .recycle_bin_enabled }} recycle{{ end }}
...
# RECYCLE:{{ .recycle_bin_enabled }} {{if .recycle_bin_enabled }}
                ...
                {{- if eq $dd.share $ndisk -}}
                        ...
                        {{- if not $dd.disabled -}}
                           ...
                           {{- if has $disk $dfdisk -}}
                                ...
                                {{- $_ := set $def "usage" "" -}}
								{{- $_ := set $dd "recycle_bin_enabled" false -}}
                           {{- else -}}
